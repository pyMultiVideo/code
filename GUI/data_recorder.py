...
class Data_recorder:
    ...
    def submit_record_new_images(self, new_images):
        """Submit a job to the video encoding ThreadPool"""
        # Submit encoding tasks to the thread pool executor for parallel processing
        future = self.camera_widget.video_capture_tab.threadpool.submit(self.record_new_images, new_images)
        self.camera_widget.video_capture_tab.threadpool_futures[
            str(self.camera_widget.label) + "_record_new_images"
        ] = future
