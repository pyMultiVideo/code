...
class BlackflyCamera(GenericCamera):
    ...
    def get_available_images(self):
        ...
        try:
            while True:
                ...
                linevalues =  PySpin.CIntegerPtr(self.nodemap.GetNode('LineStatusAll')).GetValue()
                linevalues = format(linevalues, '04b')
                # In GPIO lines: 0, 2, 3                
                gpio_buffer.append([int(linevalues[3]), int(linevalues[1]), int(linevalues[0])])
...
def list_available_cameras(VERBOSE=False) -> list[str]:
    ...
    for cam in pyspin_cam_list:
        try:
            cam.Init()
            if "Blackfly" in cam.DeviceModelName():
